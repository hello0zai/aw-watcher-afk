# Usage:
#   python3 test_continuous_events.py sd-watcher-afk-testing_{hostname}
#
# Example:
#   python3 test_continuous_events.py sd-watcher-afk-testing_erb-laptop-ubuntu
...
client = ActivityWatchClient("sd-watcher-afk-test", testing=True)
print(client.get_buckets())

bucket_id = sys.argv[1]
events = client.get_events(bucket_id)  # For example "sd-watcher-afk-testing_erb-laptop-ubuntu"
...
for event in sorted(events, key=lambda e: e.timestamp):
    if last_event:
        ...
        if not timedelta(seconds=1) > abs(diff):
            print("  WARNING: Diff had absolute value of over 1s ({}), this might be either because of a bug or due to sd-watcher-afk not running continuously".format(diff))
            wrong_events += 1
